...
if(use_factor_scores) {
  print("Since we have decided to use the factor scores, we don't need to scale the data again. We can see from the following descriptive statistics that all the factor scores have standard deviation 1 and mean 0 which is the best case for us.")
}else{
  print("Although we have decided to use original variables, we don't need to scale the data again. We can see from the following descriptive statistics that all the used variables for cluster analysis don't have high difference in range and scale.")
}
...
if(use_factor_scores) {
  print("Currently, we have decided to use the factor scores from factor analysis above which is statistically more accurate.")
}else{
  print("Currently, we have decided to use the original variable scores which has the highest loadings in each component in factor analysis above. Although this is not statistically accurate, this can be helpful in intrepretations of subsequent analysis.")
}
