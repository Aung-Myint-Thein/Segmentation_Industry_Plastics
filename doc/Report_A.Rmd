...
To be written after everything.
...
To interpret the selected factors, we often use three **Factor Technical Quality Criteria**:
...
#### Save factor scores for subsequent analysis

For simplicity, we can represent each selected factor using one of the original raw attributes, typically the one on which the factor has the highest loading on. Although it may help with the interpretation of subsequent analysis, this is not statistically as accurate. If we decide to use simple method, we would need to use following variables :

<div class="row">
<div class="col-md-4">
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, fig.align='center', results='asis'}

table_to_print <- as.data.frame(colnames(ProjectData[,segmentation_attributes_used]))
colnames(table_to_print) <- "variable"

m1<-gvisTable(table_to_print,options=list(showRowNumber=T,width=250, height=350,allowHTML=TRUE,page='disable'))
  print(m1,'chart')
```
</div>
</div>
...
However, once we decided the factors to use, we typically replace the original data with a new dataset where each observation (row) is now described not using the original raw attributes but using instead the selected factors/derived attributes. Afterall this was the goal of this analysis. 
...
if (factors_selected >=2){ 
  ...
  print(m1,'chart')
    ...
    print(xtable(NEW_ProjectData, caption="Only 1 derived variable generated", digits=3), type="html",html.table.attributes="class='table table-striped table-hover table-bordered'", caption.placement="top", comment=FALSE, include.rownames=F, include.colnames=TRUE)
    }
...
We will follow the 9 steps for the cluster analysis as below.
...
In our case the data are metric, so we continue to the next step.
...
if(use_factor_scores) {
  text_to_print <- "Since we have decided to use the factor scores, we don't need to scale the data again. We can see from the following descriptive statistics that all the factor scores have standard deviation 1 and mean 0 which is the best case for us."
}else{
  text_to_print <- "Although we have decided to use original variables, we don't need to scale the data again. We can see from the following descriptive statistics that all the used variables for cluster analysis don't have high difference in range and scale."
}
...
if(use_factor_scores) {
  text_to_print <- "Currently, we have decided to use the factor scores from factor analysis above which is statistically more accurate."
}else{
  text_to_print <- "Currently, we have decided to use the original variable scores which has the highest loadings in each component in factor analysis above. Although this is not statistically accurate, this can be helpful in intrepretations of subsequent analysis."
}
...
In our case we explore two distance metrics: the commonly used **Euclidean distance** as well as **Manhattan distance**. 
...
<div class="col-md-4">
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, fig.align='center', results='asis'}
print(xtable(euclidean_pairwise, caption="Pairwise Distances between the first 5 observations using The Euclidean Distance Metric", digits=1), type="html", html.table.attributes = "class='table table-striped table-hover table-bordered'", caption.placement="top", comment=FALSE, include.rownames=TRUE)
```
</div>
</div>
...
<div class="col-md-4">
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, fig.align='center', results='asis'}
print(xtable(manhattan_pairwise, caption="Pairwise Distances between the first 5 observations using The Manhattan Distance Metric", digits=1), type="html", html.table.attributes = "class='table table-striped table-hover table-bordered'", caption.placement="top", comment=FALSE, include.rownames=TRUE)
```
</div>
</div>
...
show_data = show_data[1:min(max_data_report,nrow(show_data)),]
m1<-gvisTable(show_data, options=list(showRowNumber=FALSE, width=350, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
</div>
</div>
...
We decided we use the `r numb_clusters_used` segments found using Kmeans as outlined above as it is more statistically accurate than hierarchical clustering. For profilling, we decided to use `r if(length(total_arributes)==length(profile_attributes_used)){"all"}else{paste(length(profile_attributes_used), " out of total ", length(total_arributes), sep="")}` original variables. Although some variables were used in factor analysis, remember that we used factor scores (not original scores) for cluster analysis. We can see how the responses to our survey differ across segments. The average values of our data for the total population as well as within each customer segment are:
...
## choose the colors for snake plots
custColors <- rainbow(12)[c(1, 2, 9, 5, 11, 12)]
...
From above table and snake plot, we can easily see that segments appear distinct with one high, one medium, and one low impact segment per specific needs. Based on this cluster analysis, we can define customer segments as :

1. Matured clients
2. Entry level clients
3. Support oriented and cash rich clients

Using the table above, we can describe the specific and differentiated needs of segments as following :

**1. Matured clients**

These customers are mature businesses and there are total `r nrow(ProjectData[ProjectData[,"Cluster_Membership"]==1,])` clients (`r round(100*nrow(ProjectData[ProjectData[,"Cluster_Membership"]==1,])/nrow(ProjectData), 2)`% of whole dataset). They needs their supplier to have an efficient and reliable ordering process (`r round(100*(cluster_profile_ratios["MAXDIFF: Efficient and reliable ordering process",1]-1),2)`% higher than population average), delivery & inventory management (`r round(100*(cluster_profile_ratios["MAXDIFF: Delivery &amp; inventory management",1]-1),2)`% higher than population average), and formula based or fixed material price (`r round(100*(cluster_profile_ratios["MAXDIFF: Formula based or fixed material price",1]-1),2)`% higher than population average). This shows that they are more mature and process based clients and they want certainty and predictability in their cost. This is strengthened more by having `r round(100*(cluster_profile_ratios["MAXDIFF: Credit limit. financing and payment terms",1]-1),2)` percent higher than population average in needs of credit limit, financing and payment terms and the highest among 3 segments.

Since they are already matured, clients from this segment do not care and need so much of collaboration for growth (`r round(100*(1-cluster_profile_ratios["MAXDIFF: Collaboration for growth",1]),2)`% lower than population average), global service and operations (`r round(100*(1-cluster_profile_ratios["MAXDIFF: Global service and operations",1]),2)`% lower than population average), business services (`r round(100*(1-cluster_profile_ratios["MAXDIFF: Business services",1]),2)`% lower than population average). They seem to know of what they want as they do not need their supplier to have broad material portfolio (`r round(100*(1-cluster_profile_ratios["MAXDIFF: Broad material portfolio",1]),2)`% lower than population average).
...
After analyzing the survey data (using factor and cluster analysis), we decided to only focus on the second segment : **"Entry level clients"** which they thought were the most important ones. They decided to perform the classification and drivers analysis using only the responses to the behavioral questions to develop a modal for placing specific customer or client in the selected segment as following :

1. "Willing to Changing suppliers"                            
2. "Willing to Give more business"                            
3. "Willing to Recommend to others"                           
4. "Willing to Specify as supplier of choice"                 
5. "Willing to Pay for consulting services"                   
6. "Willing to Pay price premium"                             
7. "Changed material suppliers in past"                       
8. "Given more business in past"                              
9. "Recommended to others in past"                            
10. "Specify as supplier of choice in past"                    
11. "Paid for consulting services in past"                     
12. "Paid premium in past"                                     
13. "Will - Order large quantity for lower price"              
14. "Will accept - No sales rep for lower price"               
15. "Will accept - Cancellation fees for lower price"          
16. "Will accept - Longer lead times for lower price"          
17. "Will accept - Broader materials specs for lower price"    
18. "Will - Use own transportation for lower price"            
19. "Will - Allocate more spending to supplier for lower price"
20. "Will - Give up support for lower price"      

#### Classification in 6 steps

We will follow the 6 steps as below for classification.
...
Since we are using variables with different scales for independent variables, we normalized our data using as following:

```{r, results='asis'}
ProjectDatafactor_Example <- apply(ProjectData,2, function(r) {if (sd(r)!=0) res=(r-mean(r))/sd(r) else res=0*r; res})
```
...
## retain the company number, sys respum, cluster membership and cluster test
ProjectData <- cbind(ProjectData[, 1:2], ProjectData_S[, -c(1,2,45,46)], ProjectData[, 45:46])
...
We will use `r estimation_data_percent`% of the dataset as estimation data (`r nrow(estimation_data)` observations), `r validation_data_percent`% of the dataset as validation data (`r nrow(validation_data)` observations), and `r test_data_percent`% as test data (`r nrow(test_data)` observations). 

<br>

#### Step 2: Setting up the dependent variable

We have merged the original dataset with cluster memerships from kmeans clustering. After that, we added one more variable at the end with the name **cluster test** which will have 1 for cluster number we want to use for classification and 0 for other clusters. Following is the first  `r max_data_report` rows of estimation data. Please go to the right end of the table to see the cluster membership and cluster test variable example.

<div class="row">
<div class="col-md-6">
```{r echo=FALSE, message=FALSE, prompt=FALSE, results='asis'}
show_data = data.frame(round(estimation_data,2))
show_data = show_data[1:min(max_data_report,nrow(show_data)),]
row<-rownames(show_data)
dfnew<-cbind(row,show_data)
change<-colnames(dfnew)
...
<div class="col-md-6">
```{r echo=FALSE, message=FALSE, prompt=FALSE, results='asis'}
class_percentages=matrix(c(sum(estimation_data[,dependent_variable]==1),sum(estimation_data[,dependent_variable]==0)), nrow=1)
colnames(class_percentages)<-c(paste("Class 1 (Cluster ", Cluster_to_test, ")", sep=""), "Class 0 (Other clusters)")
...
<div class="col-md-6">
```{r echo=FALSE, message=FALSE, prompt=FALSE, results='asis'}
class_percentages=matrix(c(sum(validation_data[,dependent_variable]==1),sum(validation_data[,dependent_variable]==0)), nrow=1)
colnames(class_percentages)<-c(paste("Class 1 (Cluster ", Cluster_to_test, ")", sep=""), "Class 0 (Other clusters)")
rownames(class_percentages)<-"# of Observations"
