...
These customers are mature businesses and there are total `r nrow(ProjectData[ProjectData[,"Cluster_Membership"]==1,])` clients (`r round(100*nrow(ProjectData[ProjectData[,"Cluster_Membership"]==1,])/nrow(ProjectData), 2)`% of whole dataset). They needs their supplier to have an efficient and reliable ordering process (`r round(cluster_profile_ratios["MAXDIFF: Efficient and reliable ordering process",1]-1,2)`% higher than population average), delivery & inventory management (`r round(cluster_profile_ratios["MAXDIFF: Delivery &amp; inventory management",1]-1,2)`% higher than population average), and formula based or fixed material price (`r round(cluster_profile_ratios["MAXDIFF: Formula based or fixed material price",1]-1,2)`% higher than population average). This shows that they are more mature and process based clients and they want certainty and predictability in their cost. This is strengthened more by having `r round(cluster_profile_ratios["MAXDIFF: Credit limit. financing and payment terms",1]-1,2)` percent higher than population average in needs of credit limit, financing and payment terms and the highest among 3 segments.

Since they are already matured, clients from this segment do not care and need so much of collaboration for growth (`r round(1-cluster_profile_ratios["MAXDIFF: Collaboration for growth",1],2)`% lower than population average), global service and operations (`r round(1-cluster_profile_ratios["MAXDIFF: Global service and operations",1],2)`% lower than population average), business services (`r round(1-cluster_profile_ratios["MAXDIFF: Business services",1],2)`% lower than population average). They seem to know of what they want as they do not need their supplier to have broad material portfolio (`r round(1-cluster_profile_ratios["MAXDIFF: Broad material portfolio",1],2)`% lower than population average).

**2. Entry level clients**

These customers are entry level business and they are total `r nrow(ProjectData[ProjectData[,"Cluster_Membership"]==2,])` clients (`r round(100*nrow(ProjectData[ProjectData[,"Cluster_Membership"]==2,])/nrow(ProjectData), 2)`% of whole dataset). Since they are entry level businesses, they are willing to collaborate for growth (`r round(cluster_profile_ratios["MAXDIFF: Collaboration for growth",2]-1,2)`% higher than population average) and they needs their supplier support towards their sustainabile goals (`r round(cluster_profile_ratios["MAXDIFF: Support towards your sustainability goals",2]-1,2)`% higher than population average). Being an entry level businesses, they also want dedicated account manager relationship (`r round(cluster_profile_ratios["MAXDIFF: Account manager relationship",2]-1,2)`% higher than population average) and they need their supplier to have a strong business services (`r round(cluster_profile_ratios["MAXDIFF: Business services",2]-1,2)`% higher than population average) and global service and operations (`r round(cluster_profile_ratios["MAXDIFF: Global service and operations",2]-1,2)`% higher than population average). 

Strangely, they are not very much interested in material coloring solutions (`r round(1-cluster_profile_ratios["MAXDIFF: Material coloring solutions",2],2)`% lower than population average) and broad material portfolio (`r round(1-cluster_profile_ratios["MAXDIFF: Broad material portfolio",2],2)`% lower than population average). This may be because they are entry level businesses and they still do not need sophisticated and advanced services.

Most importantly, they paid premium in the past (`r round(cluster_profile_ratios["Paid premium in past",2]-1,2)`% higher than population average) and therefore, they are valuable customers. However, they have highest average score in "willing to change supplier" with `r round(cluster_profile_ratios["Willing to Changing suppliers",2],2)` among all segments. So, we need to pay more attention to this segments and our subsequent analysis will be based on this segment.

**3. Support oriented and cash rich clients**

These customers are support oriented and cash rich clients and they are total `r nrow(ProjectData[ProjectData[,"Cluster_Membership"]==3,])` clients (`r round(100*nrow(ProjectData[ProjectData[,"Cluster_Membership"]==3,])/nrow(ProjectData), 2)`% of whole dataset). They rate highly generally on support related questions such as material selection support (`r round(cluster_profile_ratios["MAXDIFF: Material selection support",3]-1,2)`% higher than population average), processing support (`r round(cluster_profile_ratios["MAXDIFF: Processing support",3]-1,2)`% higher than population average), and application development support (`r round(cluster_profile_ratios["MAXDIFF: Application development support",3]-1,2)`% higher than population average). Interestingly, they need supplier with significantly broad material portfolio (`r round(cluster_profile_ratios["MAXDIFF: Broad material portfolio",3]-1,2)`% higher than population average) and material coloring solutions (`r round(cluster_profile_ratios["MAXDIFF: Material coloring solutions",3]-1,2)`% higher than population average). May be because their need of products are diversed and therefore, they rate less for fixed material price (check below). As support oriented clients, they are more interested to collaborate during innovation process (`r round(cluster_profile_ratios["MAXDIFF: Collaboration during your innovation process",3]-1,2)`% higher than population average) than collaboration for growth (`r round(1-cluster_profile_ratios["MAXDIFF: Collaboration for growth",3],2)`% lower than population average). 

They definitely are cash rish clients. They rate generally lower than population average for cost related needs such as total system cost reduction (`r round(1-cluster_profile_ratios["MAXDIFF: Total system cost reduction",3],2)`% lower than population average) and formula based or fixed material price (`r round(1-cluster_profile_ratios["MAXDIFF: Formula based or fixed material price",3],2)`% lower than population average). They do not need credit limit, financing and payment terms (`r round(1-cluster_profile_ratios["MAXDIFF: Credit limit. financing and payment terms",3],2)`% lower than population average). This segment of clients paid premium the most among all the segments (`r round(cluster_profile_ratios["Paid premium in past",3]-1,2)`% higher than population average) and least willing to change supplier (`r round(1-cluster_profile_ratios["Willing to Changing suppliers",3],2)`% lower than population average).
